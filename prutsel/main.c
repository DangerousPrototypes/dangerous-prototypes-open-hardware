...
void bpMsg(unsigned int address, int length);
...
{	
	...
	bpWline("Buspirate new messageblaat");
	bpWline("(C)hris 2010");
...
{	int i;
	...
	{	if((i%3)==0)  UART1TX(__builtin_tblrdl((int)&messages+((i/3)<<1))&0x00FF);
		if((i%3)==1) UART1TX((__builtin_tblrdl((int)&messages+((i/3)<<1))>>8)&0xFF);
		if((i%3)==2)  UART1TX(__builtin_tblrdh((int)&messages+((i/3)<<1))&0x00FF);
	}
