...
uint32_t PIC24_ReadFlash(struct picprog_t *p, uint8_t *fw_data)
{
	...
	for (page = 0; page < pic->memmap[PIC_MEM_FLASH].size / fam->page_size; page++)
	{
		...
		iface->PIC424Write(0x200000 | ((u_addr & 0xffff0000) >> 12), 0, 0);//SIX,0x200FF0,5, N/A MOV #<SourceAddress23:16>, W0
		iface->PIC424Write(0x880190, 0, 0);//SIX,0x880190,5, N/AMOV W0, TBLPAG
		iface->PIC424Write(0x200006 | ((u_addr & 0x000ffff) << 4), 0, 2);//SIX,0x200006,5, N/A MOV #<SourceAddress15:0>, W6
		iface->PIC424Write(0x207847, 0, 1);//SIX,0x200007,5, N/A  //MOV #VISI,W7 //PIC24NOP();//SIX,0x000000,5, N/A
		iface->flush();
		...
		for(i=0; i<fam->page_size; i=i+8){
            ...
            //2XX/3XX
            //X54
            iface->PIC424Read(0xBA0B96, &r, 2);
            fw_data[(page * fam->page_size)+i]=(uint8_t)r;
            fw_data[(page * fam->page_size)+i+1]=(uint8_t)(r>>8);

            iface->PIC424Write(0xBADBB6, 0, 2);//SIX,0x200007,5, N/A  //MOV #VISI,W7 //PIC24NOP();//SIX,0x000000,5, N/A
            iface->flush();
            iface->PIC424Read(0xBAD3D6, &r, 2);
            fw_data[(page * fam->page_size)+i+2]=r;
            fw_data[(page * fam->page_size)+i+6]=(uint8_t)(r>>8);

            iface->PIC424Read(0xBA0BB6, &r, 2);
            ...
            */
            ...
            //2XX/3XX
            //X54
			iface->PIC424Read(0xBA0B96, &buf, 1);//read one 2 word chunk
...
/*
    for(ctr=0; ctr<(pic->memmap[PIC_MEM_FLASH].size); ctr=ctr+16){
        //X10
        //2XX/3XX
        //X54
