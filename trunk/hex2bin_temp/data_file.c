 ...
 */
 //Data_Checksum(&out_buf[addr+i], byte_count);
uint8_t Data_Checksum(uint8_t *buf, uint16_t size) {
	uint16_t i;
	uint8_t sum = 0;
  //  printf("size= %i \n",size);
	for (i=0; i<size; i++) {
	//    printf("sum : %i  buf[%i]= %2X \n",sum,i,*buf[i]);
		sum -= buf[i];
	}
	return sum;
}
...
uint32_t HEX_ReadFile(const char *file, uint8_t *out_buf, uint32_t out_buf_size) {
	...
	while(fgets(raw_line, sizeof(raw_line), fp) != 0) {
		// read line header
		unsigned int tmp[3];
		uint8_t byte_count,datachecksum;
		uint32_t addr;
		uint8_t rec_type;
		...
		//	printf("\nbefore: addr= %4X  base_addr = %4x line %s\n",addr,base_addr,raw_line);

			addr = base_addr | addr;
		//	printf("After: addr= %4X  base_addr = %4x  line %s\n",addr,base_addr,raw_line);


		//	 printf("Out_buf_size= %i  byte_count = %i addr = %i\n",out_buf_size,byte_count,addr);
           if (out_buf_size < byte_count + addr) {
                printf("\nError found before this line [%i]--> %s\n",line,raw_line);
			    printf("Out_buf_size= %i  byte_count = %i addr = %i\n",out_buf_size,byte_count,addr);
				printf("Error Message: Data won't fit into buffer :( \n");
				fclose(fp);
				return 0;
			}
			...
			chksum = tmp[0];
			// TODO: check chksum
	//		printf("Out_buff[%i+%i]=%u\n",addr,i,out_buf[addr+i]);
			datachecksum=Data_Checksum(out_buf2, byte_count);
			//datachecksum=Data_Checksum(&out_buf[addr+i], byte_count);
			if (datachecksum != chksum) {
		//		printf("WARNING: HEX checksum error on line %d !! Expected chksum is %d but returned %u \n", line,chksum,datachecksum );
			}

		} else if (rec_type == 0x04) {
			// base addr
			sscanf(raw_line+9, "%4x", &base_addr);
		//	printf("line: %s  base address= %u \n",raw_line,base_addr);
			base_addr <<= 16;
		//	printf("ROL <<==%u\n",base_addr);
