...
void cdc_rx( void ) {
	unsigned int i;
	rxbdp = bdp;			// TODO: Implement PP buffering, move to API
	data = bdp->BDADDR;
	data_end = bdp->BDADDR + bdp->BDCNT;
	DPRINTF("cdc_rx %u\t", bdp->BDCNT);
	for (i=0; i<bdp->BDCNT; i++)
		DPRINTF("%c ", bdp->BDADDR[i]);
	DPRINTF("\n");
}

void cdc_tx( void ) {
	DPRINTF("cdc_tx 0x%02X %u\n", bdp->BDSTAT, bdp->BDCNT);
	bdp->BDCNT = 0;
	txbdp = bdp;			// TODO: Implement PP buffering, move to API
}
...
char BusyCDC( void ) {
	return txbdp->BDSTAT & UOWN;	// TODO: Implement PP buffering
}
