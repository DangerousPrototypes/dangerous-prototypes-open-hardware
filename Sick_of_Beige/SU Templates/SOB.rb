...
$slotToEdgeDistance = 2*$acrylicThichness		# For making the case strong enough
...
def SOB.create_top_bottom(x,y,z)
	...
	model.start_operation $exStrings.GetString("Create top/bottom piece")
    ...
    bottom.pushpull -$acrylicThichness
	
	# Create the mounting holes
	# We couldn't create a new face on an existing face, +1.m is for that, kind of a cheat :)
	holes = []
	holes[0] = centerPt + [-$pcbWidth/2,$pcbLength/2,0] + [$holeToEdgeDistance,-$holeToEdgeDistance,$acrylicThichness]
	holes[1] = centerPt + [$pcbWidth/2,$pcbLength/2,0] + [-$holeToEdgeDistance,-$holeToEdgeDistance,$acrylicThichness]
	holes[2] = centerPt + [-$pcbWidth/2,-$pcbLength/2,0] + [$holeToEdgeDistance,$holeToEdgeDistance,$acrylicThichness]
	holes[3] = centerPt + [$pcbWidth/2,-$pcbLength/2,0] + [-$holeToEdgeDistance,$holeToEdgeDistance,$acrylicThichness]
	
	holes.each do |i|
		circle = entities.add_circle i, [0,0,1], $holeDiameter/2
		circle[0].faces[0].pushpull -$acrylicThichness
	end
	...
	if $sideType > 0
		...
		wSlots = []
		eSlots = []
		nSlots[0] = centerPt + [-$pcbWidth/2,$pcbLength/2,0] + [0,$pcbToSideDistance,$acrylicThichness] + [emptySpaceW/2,0,0]
		sSlots[0] = centerPt + [$pcbWidth/2,-$pcbLength/2,0] + [0,-$pcbToSideDistance,$acrylicThichness] + [-emptySpaceW/2,0,0]
		wSlots[0] = centerPt + [-$pcbWidth/2,-$pcbLength/2,0] + [-$pcbToSideDistance,0,$acrylicThichness] + [0,emptySpaceL/2,0]
		eSlots[0] = centerPt + [$pcbWidth/2,$pcbLength/2,0] + [$pcbToSideDistance,0,$acrylicThichness] + [0,-emptySpaceL/2,0]
		...
		for i in 0...numTabsW
			...
			ptsN[2] = nSlots[i] + [$slotLength,$acrylicThichness,0]
			ptsN[3] = nSlots[i] + [0,$acrylicThichness,0]
			slotN = entities.add_face ptsN
			slotN.pushpull -$acrylicThichness

			ptsS = []
			ptsS[0] = sSlots[i]
			ptsS[1] = sSlots[i] + [-$slotLength,0,0]
			ptsS[2] = sSlots[i] + [-$slotLength,-$acrylicThichness,0]
			ptsS[3] = sSlots[i] + [0,-$acrylicThichness,0]
			slotS = entities.add_face ptsS
			slotS.pushpull -$acrylicThichness
		end
		...
		for i in 0...numTabsL
			...
			ptsW[2] = wSlots[i] + [-$acrylicThichness,$slotLength,0]
			ptsW[3] = wSlots[i] + [-$acrylicThichness,0,0]
			slotW = entities.add_face ptsW
			slotW.pushpull -$acrylicThichness

			ptsE = []
			ptsE[0] = eSlots[i]
			ptsE[1] = eSlots[i] + [0,-$slotLength,0]
			ptsE[2] = eSlots[i] + [$acrylicThichness,-$slotLength,0]
			ptsE[3] = eSlots[i] + [$acrylicThichness,0,0]
			slotE = entities.add_face ptsE
			slotE.pushpull -$acrylicThichness
		end
...
def SOB.create_width_side1
	# Save operation for ease of Undo
	model = Sketchup.active_model
	model.start_operation $exStrings.GetString("Create width side piece")
	...
	panel = []
	panel[0] = Geom::Point3d.new(0,0,0) + [-$pcbWidth/2,$pcbLength/2,0] + [-$pcbToSideDistance,$pcbToSideDistance,$acrylicThichness] + [-$acrylicThichness,0,0]
	panel[1] = Geom::Point3d.new(0,0,0) + [$pcbWidth/2,$pcbLength/2,0] + [$pcbToSideDistance,$pcbToSideDistance,$acrylicThichness]
	panel[2] = panel[1] + [0,0,$areaHeight/4]
	panel[3] = panel[2] + [$acrylicThichness,0,0]
	...
	for i in 0...$longSlot.length
		...
		pts[2] = $longSlot[i] + [$slotLength,$acrylicThichness,0]
		pts[3] = $longSlot[i] + [0,$acrylicThichness,0]
		tab = entities.add_face pts
		tab.pushpull $acrylicThichness
	end
	...
	for i in 0...$longSlot.length
		...
		pts[2] = $longSlot[i] + [0,0,$areaHeight] + [$slotLength,$acrylicThichness,0]
		pts[3] = $longSlot[i] + [0,0,$areaHeight] + [0,$acrylicThichness,0]
		tab = entities.add_face pts
		tab.pushpull $acrylicThichness
	end
...
def SOB.create_length_side1
	...
	# Create the side panel
	panel = []
	panel[0] = Geom::Point3d.new(0,0,0) + [-$pcbWidth/2,-$pcbLength/2,0] + [-$pcbToSideDistance,-$pcbToSideDistance,$acrylicThichness] + [0,-$acrylicThichness,0]
	panel[1] = Geom::Point3d.new(0,0,0) + [-$pcbWidth/2,$pcbLength/2,0] + [-$pcbToSideDistance,$pcbToSideDistance,$acrylicThichness]
	panel[2] = panel[1] + [0,0,$areaHeight/4]
	panel[3] = panel[2] + [0,$acrylicThichness,0]
	panel[4] = panel[3] + [0,0,$areaHeight/2]
	panel[5] = panel[2] + [0,0,$areaHeight/2]
	panel[6] = panel[1] + [0,0,$areaHeight]
	panel[7] = panel[0] + [0,0,$areaHeight]
	panel[8] = panel[7] + [0,0,-$areaHeight/4]
	panel[9] = panel[8] + [0,$acrylicThichness,0]
	panel[10] = panel[9] + [0,0,-$areaHeight/2]
	panel[11] = panel[0] + [0,0,$areaHeight/4]
	side = entities.add_face panel
	side.pushpull -$acrylicThichness
	...
	for i in 0...$shortSlot.length
		...
		pts[2] = $shortSlot[i] + [-$acrylicThichness,$slotLength,0]
		pts[3] = $shortSlot[i] + [-$acrylicThichness,0,0]
		tab = entities.add_face pts
		tab.pushpull $acrylicThichness
	end
	...
	for i in 0...$shortSlot.length
		...
		pts[2] = $shortSlot[i] + [0,0,$areaHeight] + [-$acrylicThichness,$slotLength,0]
		pts[3] = $shortSlot[i] + [0,0,$areaHeight] + [-$acrylicThichness,0,0]
		tab = entities.add_face pts
		tab.pushpull $acrylicThichness
	end
...
def SOB.create_width_side2
	...
	panel = []
	panel[0] = Geom::Point3d.new(0,0,0) + [-$pcbWidth/2,$pcbLength/2,0] + [-$pcbToSideDistance,$pcbToSideDistance,$acrylicThichness] + [-$acrylicThichness,0,0]
	panel[1] = Geom::Point3d.new(0,0,0) + [$pcbWidth/2,$pcbLength/2,0] + [$pcbToSideDistance,$pcbToSideDistance,$acrylicThichness] + [$acrylicThichness,0,0]
	panel[2] = panel[1] + [0,0,$areaHeight/2]
	panel[3] = panel[2] + [-$acrylicThichness,0,0]
	panel[4] = panel[3] + [0,0,$areaHeight/2]
	panel[5] = panel[0] + [$acrylicThichness,0,$areaHeight] 
	panel[6] = panel[0] + [$acrylicThichness,0,$areaHeight/2]
...
def SOB.create_length_side2
	...
	# Create the side panel
	panel = []
	panel[0] = Geom::Point3d.new(0,0,0) + [-$pcbWidth/2,-$pcbLength/2,0] + [-$pcbToSideDistance,-$pcbToSideDistance,$acrylicThichness]
	panel[1] = Geom::Point3d.new(0,0,0) + [-$pcbWidth/2,$pcbLength/2,0] + [-$pcbToSideDistance,$pcbToSideDistance,$acrylicThichness]
	panel[2] = panel[1] + [0,0,$areaHeight/2]
	panel[3] = panel[2] + [0,$acrylicThichness,0]
	panel[4] = panel[3] + [0,0,$areaHeight/2]
	panel[5] = panel[0] + [0,-$acrylicThichness,$areaHeight] 
	panel[6] = panel[0] + [0,-$acrylicThichness,$areaHeight/2] 
	panel[7] = panel[0] + [0,0,$areaHeight/2]
	side = entities.add_face panel
	side.pushpull -$acrylicThichness
...
def SOB.main
	...
	case $sideType
		when 0
			puts "No side panels are created."
		when 1
		...
		when 2
			...
			puts "Invalid setting! No side panels will be created!"
		end
	SOB.create_top_bottom(0,0,$areaHeight+$acrylicThichness)
end

def SOB.dialog
	prompts = ["PCB Width (mm)", "PCB Length (mm)", "Circuit Height (mm)", "Acrylic Thickness (mm)", "Side Panel Style"]
	defaults = [String($pcbWidth/1.m), String($pcbLength/1.m), String($areaHeight/1.m), String($acrylicThichness/1.m), "None"]
	...
	$acrylicThichness = Float(input[3]).m
	case input[4]
		when "None"
			$sideType = 0
		when "Interlocking"
