...
#define OS_NAME(tok) STR_EXPAND(tok)
	...
	int p_write(int fd, const void* buf, int len)
	{
	...
	int p_read(int fd, void* buf, int len)
	{
		HANDLE hCom = (HANDLE)fd;
		int res = 0;
	...
	int p_open(const char* path, unsigned long flags)
	{
...
CmdLineArgs cmdLineArgs={0,0,0,NULL,NULL};
...
int sendCommandAndWaitForResponse(int fd, uint8 *command)
{
	...
	res = p_write(fd, command, HEADER_LENGTH + command[LENGTH_OFFSET]);
...
{
	return p_open(dev, O_RDWR | O_NOCTTY | O_NDELAY | flags);
}
...
int main (int argc, const char** argv)
{
	...
	//send HELLO
	res = p_write(dev_fd, BOOTLOADER_HELLO_STR, 1);
