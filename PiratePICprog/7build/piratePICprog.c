...
typedef struct
{
	...
	const char* g_hexfile_path ;
	int		dev_fd;
}CmdLineArgs;

CmdLineArgs cmdLineArgs={0,0,0,NULL,NULL,-1};
...
int parseCommandLine(int argc, const char** argv)
{
	...
	for(i=1; i<argc; i++)
	{
		...
		if( !strncmp(argv[i], "--hex=", 6) ) {
			cmdLineArgs.g_hexfile_path = argv[i] + 6;
		} else if ( !strncmp(argv[i], "--dev=", 6) ) {
			cmdLineArgs.g_device_path = argv[i] + 6;
		} else if ( !strcmp(argv[i], "--verbose") ) {
			cmdLineArgs.g_verbose = 1;
		} else if ( !strcmp(argv[i], "--hello") ) {
			cmdLineArgs.g_hello_only = 1;
		} else if ( !strcmp(argv[i], "--simulate") ) {
			cmdLineArgs.g_simulate = 1;
...
int main (int argc, const char** argv)
{
	...
	if( !cmdLineArgs.g_hello_only ) {
		
		if( !cmdLineArgs.g_hexfile_path ) {
		...
		printf("Parsing HEX file [%s]\n", cmdLineArgs.g_hexfile_path);
		
		res = readHEX(cmdLineArgs.g_hexfile_path, bin_buff, (256 << 8), pages_used);
	...
	if( !cmdLineArgs.g_device_path ) {
		fprintf(stderr, "Please specify serial device path --dev=/dev/...\n");
		return -1;
	}
	
	printf("Opening serial device %s...", cmdLineArgs.g_device_path);
	
	cmdLineArgs.dev_fd = openPort(cmdLineArgs.g_device_path, 0);
	
	if( cmdLineArgs.dev_fd < 0 ) {
		puts("ERROR");
		fprintf(stderr, "Could not open %s\n", cmdLineArgs.g_device_path);
		goto Error;
	}
	puts("OK");
	...
	if( configurePort(cmdLineArgs.dev_fd, B115200) < 0 ) {
		puts("ERROR");
	...
	sendString(cmdLineArgs.dev_fd, 1, "\x00");
	res = readWithTimeout(cmdLineArgs.dev_fd, buffer, 5, 1);
	if( memcmp(buffer, "BBIO1", 5) ) {
		for(i=0; i<20; i++){
			buffer[i]=0x00;
		}
		sendString(cmdLineArgs.dev_fd, 19, buffer);
	...
	sendString(cmdLineArgs.dev_fd, 1, "\x05");
	res = readWithTimeout(cmdLineArgs.dev_fd, buffer, 4, 1);
	buffer[4]=0x00;
	printf(buffer);
	...
	if( !cmdLineArgs.g_hello_only ) {
		...
		res = sendFirmware(cmdLineArgs.dev_fd, bin_buff, pages_used);
