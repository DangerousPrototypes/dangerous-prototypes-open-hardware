...
uint32_t PIC24_EnterICSP(struct picprog_t *p, icsp_t type) {
...
uint32_t PIC24_ReadID(struct picprog_t *p) {
	...
	iface.PIC24_Read(opts, 0x00FF0000, &id, 2); //give it the ID addres

	return id;
}

uint32_t PIC24_Read(struct picprog_t *p, uint32_t addr, uint16_t* Data, int length) {
	struct iface_t *iface = p->iface;
	void *opts = p->iface_data;
	int ctr, nopctr=0;
...
uint32_t PIC24_Write(struct picprog_t *, uint32_t tblptr, uint16* Data, int length) {
	...
	while(1) {
		...
		VISI = ifacePIC424Read(opts);//REGOUT,0x000000,5, read VISI register (PIC includes 2 NOPS after every read, may need to be updated later)
